...
WHERE C.username = $1 AND (SELECT SUM( \
        ...
        get_salary_list: 'SELECT C.username AS cusername, COALESCE((SELECT SUM( 
            CASE 
                WHEN DATE_PART('month', TIMESTAMP '2020-10-15') = DATE_PART('month', start_period::date) AND (DATE_PART('month', TIMESTAMP '2020-10-15') < DATE_PART('month', end_period::date) OR DATE_PART('year', TIMESTAMP '2020-10-15') < DATE_PART('year', end_period::date)) THEN 
                    DATE_PART('day', (date_trunc('month', start_period::date) + interval '1 month') - start_period::date) * bid_rate * 0.75
                WHEN DATE_PART('month', TIMESTAMP '2020-10-15') = DATE_PART('month', end_period::date) AND (DATE_PART('month', TIMESTAMP '2020-10-15') > DATE_PART('month', start_period::date) OR DATE_PART('year', TIMESTAMP '2020-10-15') > DATE_PART('year', end_period::date)) THEN 
                    DATE_PART('day', end_period::date) * bid_rate * 0.75
                WHEN DATE_PART('month', start_period::date) = DATE_PART('month', TIMESTAMP '2020-10-15') AND DATE_PART('month', end_period::date) = DATE_PART('month', TIMESTAMP '2020-10-15') AND DATE_PART('year', start_period::date) = DATE_PART('year', end_period::date) THEN 
                    (end_period::date - start_period::date + 1) * bid_rate * 0.75
                WHEN DATE_PART('month', start_period::date) < DATE_PART('month', TIMESTAMP '2020-10-15') AND DATE_PART('month', end_period::date) > DATE_PART('month', TIMESTAMP '2020-10-15') AND DATE_PART('year', start_period::date) <= DATE_PART('year', TIMESTAMP '2020-10-15') AND DATE_PART('year', end_period::date) >= DATE_PART('year', TIMESTAMP '2020-10-15') THEN 
                    DATE_PART('day', date_trunc('month', TIMESTAMP '2020-10-15') + interval '1 month' - interval '1 day') * bid_rate * 0.75
            END) 
            FROM bookings 
   ...
   SELECT C.username AS cusername, 3000 + 0.8 * (SELECT SUM( 
            CASE 
                ...
                WHEN DATE_PART('month', start_period::date) = DATE_PART('month', TIMESTAMP '2020-10-15') AND DATE_PART('month', end_period::date) = DATE_PART('month', TIMESTAMP '2020-10-15') AND DATE_PART('year', start_period::date) = DATE_PART('year', end_period::date) THEN 
                    (end_period::date - start_period::date + 1) * bid_rate
                WHEN DATE_PART('month', start_period::date) < DATE_PART('month', TIMESTAMP '2020-10-15') AND DATE_PART('month', end_period::date) > DATE_PART('month', TIMESTAMP '2020-10-15') AND DATE_PART('year', start_period::date) <= DATE_PART('year', TIMESTAMP '2020-10-15') AND DATE_PART('year', end_period::date) >= DATE_PART('year', TIMESTAMP '2020-10-15') THEN 
                    DATE_PART('day', date_trunc('month', TIMESTAMP '2020-10-15') + interval '1 month' - interval '1 day') * bid_rate
            END) 
   FROM bookings 
   WHERE caretaker = C.username AND status = 'ACCEPTED')/(SELECT SUM( 
            CASE 
                ...
                WHEN DATE_PART('month', start_period::date) = DATE_PART('month', TIMESTAMP '2020-10-15') AND DATE_PART('month', end_period::date) = DATE_PART('month', TIMESTAMP '2020-10-15') AND DATE_PART('year', start_period::date) = DATE_PART('year', end_period::date) THEN 
                    (end_period::date - start_period::date + 1)
                WHEN DATE_PART('month', start_period::date) < DATE_PART('month', TIMESTAMP '2020-10-15') AND DATE_PART('month', end_period::date) > DATE_PART('month', TIMESTAMP '2020-10-15') AND DATE_PART('year', start_period::date) <= DATE_PART('year', TIMESTAMP '2020-10-15') AND DATE_PART('year', end_period::date) >= DATE_PART('year', TIMESTAMP '2020-10-15') THEN 
                    DATE_PART('day', date_trunc('month', TIMESTAMP '2020-10-15') + interval '1 month' - interval '1 day')
            END) 
   FROM bookings 
   WHERE caretaker = C.username AND status = 'ACCEPTED') * ((SELECT SUM( 
            CASE 
